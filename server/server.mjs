...
const AGENT_TOOLS = [
	...
	},
	{
		...
		function: {
			name: 'read_chunks',
			description:
				"Force la (re)lecture de chunks precis du tableau (par leur numero), MEME s'ils sont deja dans ta memoire. A utiliser si tu as besoin de revoir une zone precise (relire une equation, verifier un detail). Le tableau est decoupe en chunks de 450x450 ancres a l'origine : le chunk (i,j) couvre x de i*450 a (i+1)*450, y de j*450 a (j+1)*450.",
			parameters: {
				type: 'object',
				properties: {
					chunks: {
						type: 'array',
						description: 'Numeros des chunks a relire (i, j).',
						items: {
							type: 'object',
							properties: {
								i: { type: 'integer' },
...
function buildManifest(view, tiles, chunks) {
	...
	if (Array.isArray(tiles) && tiles.length) {
		...
		tiles.forEach((t, i) => {
			if (t.kind === 'overview') {
				lines.push(`${i + 1}. Vue d'ensemble de toute la region visible.`)
			} else {
				const ci = r(t.x / 450)
				const cj = r(t.y / 450)
				lines.push(
					`${i + 1}. chunk (${ci},${cj}) — zone x ${r(t.x)}..${r(t.x + t.w)}, y ${r(t.y)}..${r(t.y + t.h)}.`
				)
			}
		})
	} else {
